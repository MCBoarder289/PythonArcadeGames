...
def draw_cloud(cloudname):
    """
    Draws a cloud, given the associative name in the dictionary.
    ...
    y_offset = 0
    for i in range(2):
        pygame.draw.ellipse(screen, CLOUDS, [cloud_elements[cloudname][0] + x_offset,  # cloud_elements index pull
                                             cloud_elements[cloudname][1] + y_offset,  # cloud_elements index pull
...
for i in range(25):
    left_trees["tree{0}".format(i)] = (random.randrange(-5, trees_left_xlim),  # Random x coordinate
                                       ...
                                       random.uniform(trees_mult_low, trees_mult_high))  # Random multiplier for size

    right_trees["tree{0}".format(i)] = (random.randrange(trees_right_xlim, 805),
                                        random.randrange(trees_y_upperlim, trees_y_lowerlim),
                                        random.uniform(trees_mult_low, trees_mult_high))
...
while not done:
    ...
    for item in left_trees.items():
        draw_tree(item)

    for item in right_trees.items():
        draw_tree(item)
